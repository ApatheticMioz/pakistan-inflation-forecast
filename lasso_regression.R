# --- Lasso Regression Modeling, Forecasting & Evaluation (v5 - lambda.1se, Console Output) ---
...
results_folder <- file.path(base_dir, "Model_Results/Lasso_v5_1se") # Updated folder
...
# --- 4. Lasso Regression Model Training (alpha=1, lambda.1se) ---
cat("\n--- Training Lasso Regression Model (alpha=1, lambda.1se) ---\n")
set.seed(123)
cv_lasso <- cv.glmnet(x_train, y_train, alpha = 1, nfolds = min(10, nrow(x_train)))
# --- MODIFICATION: Use lambda.1se ---
best_lambda_lasso <- cv_lasso$lambda.1se
cat("Best Lambda (lambda.1se) found via CV on training data:", best_lambda_lasso, "\n")
cat("Lambda Min was:", cv_lasso$lambda.min, "\n")

plot(cv_lasso); title("Lasso Regression CV (Training Data)", line = 2.5)
abline(v=log(cv_lasso$lambda.1se), lty=2, col="blue") # Add line for lambda.1se
...
cat("\nFinal Lasso Model Coefficients (lambda.1se):\n")
lasso_coeffs <- coef(lasso_model); print(lasso_coeffs)
selected_vars <- rownames(lasso_coeffs)[which(lasso_coeffs != 0)]; selected_vars <- selected_vars[selected_vars != "(Intercept)"]
cat("\nVariables selected by Lasso (non-zero coefficients):\n"); print(selected_vars)

# --- 5. Prediction on Test Set ---
cat("\n--- Generating Predictions on Test Set ---\n")
predictions_test_scaled <- predict(lasso_model, s = best_lambda_lasso, newx = x_test)
...
# In-Sample R-squared
predictions_train_scaled <- predict(lasso_model, s = best_lambda_lasso, newx = x_train)
...
evaluation_metrics <- data.frame( Metric = c("Lambda_1se", "RMSE_scaled", "MAE_scaled", "RMSE_unscaled", "MAE_unscaled", "MAPE_unscaled", "R_squared_train"),
                                  Value = c(best_lambda_lasso, rmse_scaled, mae_scaled, rmse_unscaled, mae_unscaled, mape_unscaled, r_squared_train) )
...
# (Same plotting logic)
# ... [Insert plotting code here, update titles] ...
# --- START Copied Plots ---
...
print( ggplot(test_data_unscaled, aes(x = .data[[target_col_final]], y = Predicted_Unscaled)) + geom_point(alpha = 0.7) + geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "red") +
         labs(title = "Lasso (lambda.1se): Predicted vs Actual CPI (Test Set - Original Scale)", subtitle = "Note: Unscaling might be inaccurate", x = paste("Actual", target_col_final), y = paste("Predicted", target_col_final)) + theme_minimal() + coord_equal() )
...
print( ggplot(plot_data_ts, aes(x = Date, y = Value, color = Type)) + geom_line(data = filter(plot_data_ts, Type %in% c("Train Actual", "Test Actual")), alpha = 0.8) +
         geom_line(data = filter(plot_data_ts, Type == "Test Predicted"), linetype = "dashed", alpha = 0.8) + labs(title = "Lasso (lambda.1se): Headline CPI Forecast vs Actuals (Original Scale)", x = "Date", y = target_col_final, color = "Data Type") +
...
cat("\n--- Lasso Regression Analysis Finished (v5 - lambda.1se, Console Output) --- \n")